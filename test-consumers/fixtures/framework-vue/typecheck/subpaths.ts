/**
 * The other two Svelte-free subpaths a Vue consumer reaches for:
 * `triiiceratops/selectors` (the framework-neutral runtime) and
 * `triiiceratops/testing` (the headless test kit). Same rules as `vue.ts` —
 * no Svelte installed, `skipLibCheck: false`, `types: []` — so a Svelte type in
 * either declaration graph fails this fixture's build.
 *
 * Together with the React fixture's `typecheck/`, all four subpaths named by the
 * promise (`./react`, `./vue`, `./selectors`, `./testing`) are covered. This
 * file is byte-identical to its React counterpart apart from this paragraph:
 * both fixtures cover these two subpaths so that either one failing is enough
 * to fail the packed run.
 */
