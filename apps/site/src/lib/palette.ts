 ...
 * `app.css` to the same values — the stylesheet renders the palette and cannot
 * import from here, so the agreement is asserted rather than assumed.
 */
 ...
 *
 * Exhaustive by test: a hex-valued custom property in `app.css`'s `:root` that
 * is missing here fails the unit suite, so the appendix cannot fall behind the
 * stylesheet.
 */
...
export const PAIRINGS: readonly Pairing[] = [
    ...
    { ink: '--ink-on-dark', ground: '--ink-block', role: 'The dark link block' },
    {
        ink: '--ink-dim-on-dark',
        ground: '--ink-block',
