<script lang="ts">
    ...
    // The page's light/dark scheme is split from the viewer's theme.
    //
    // The scheme itself belongs to the site: `$lib/theme` owns the key, and
    // `app.html`'s blocking script has already applied it to `<html data-theme>`
    // before this component exists. `pageTheme` only mirrors that choice so the
    // viewer can follow it, and the rail's toggle is what changes it.
    //
    // `viewerTheme` is what we hand to the viewer component: it follows
    // `pageTheme` until the reader explicitly picks one of the four built-in
    // themes for the viewer (via the config pane). Once that happens
    // (`viewerThemeUserSet`), the page's light/dark toggle no longer steers it.
    const initialPageTheme = currentTheme();
    let pageTheme = $state<Theme>(initialPageTheme);

    let viewerThemeUserSet = $state(false);
    let viewerThemeExplicit = $state<BuiltInTheme>(initialPageTheme);
    let viewerTheme = $derived<BuiltInTheme>(
        viewerThemeUserSet ? viewerThemeExplicit : pageTheme,
    );
    ...
    // Defines <triiiceratops-viewer>. A side-effect entrypoint: it resolves to
    // the element bundle, which declares no exports and no types, so TypeScript
    // reads the JavaScript and reports it as not a module.
    // @ts-expect-error - the register entrypoint has nothing to export
...
<style>
    ...
    /* A step down from the site's reading size: the chrome is dense with
       controls, and the site's own value is for prose. */
    .demo-root {
        min-height: 100dvh;
        height: 100dvh;
        background: var(--bone);
