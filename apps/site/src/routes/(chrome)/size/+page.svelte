...
<section class="band" aria-labelledby="capability">
    ...
    <p class="explain">
        The capability axis counts how many IIIF Cookbook recipes each viewer
        fully supports, of the {RECIPES.total} distinct recipes in the
        <a href={MATRIX_URL}>official support matrix</a>, {RECIPES.audiovisual} of
        them audiovisual. Every viewer but this one is counted from its own row in
        that matrix; ours is counted from the recipe catalog this site is built from,
        which is the stricter of the two — the disclosure below gives both figures.
        Both axes have to describe the same viewer, so the size axis is each project's
        audiovisual session.
    </p>
...
<div class="band">
    <details class="disclose">
        ...
        <h3>How the recipe figure is counted</h3>
        <p class="explain">
            ...
            the figure plotted at {self?.recipes}. That count is binary. A
            recipe whose own feature does not happen is counted as unsupported
            however much of its manifest still renders, so no Triiiceratops
            figure on this page carries a partial, and the ones that do belong
            to viewers whose matrix rows record them.
        </p>
        <p class="explain">
            The two axes are not read the same way, and the difference runs
            against us. Every other point is that project's own row in the <a
                href={MATRIX_URL}>support matrix</a
            >; ours is counted from the recipe catalog this site is built from.
            The matrix's Triiiceratops row records {RECIPES.matrix} of {RECIPES.total},
            because it holds what a project has submitted to it rather than what
            the project currently does. Reading our point off the matrix instead
            would move it left, not right.
        </p>
