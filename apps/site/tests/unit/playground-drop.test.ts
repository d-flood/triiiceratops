/**
 * The playground's drop seam. Nothing drives the playground in a browser, and
 * jsdom implements neither `DataTransfer` nor `DragEvent`, so the handler is
 * tested where it is a function of a payload rather than of an event.
 */

import { describe, expect, it } from 'vitest';
import type { DropPayloadSource } from '@triiiceratops/config';

import { resolveDroppedView } from '$lib/playground/drop';
