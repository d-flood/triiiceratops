/**
 * The drop seam a host with its own manifest selection uses. jsdom implements
 * neither `DataTransfer` nor `DragEvent`, so the handler is tested where it is
 * a function of a payload rather than of an event.
 */

import { describe, expect, it } from 'vitest';
import type { DropPayloadSource } from '@triiiceratops/config';

import { resolveDroppedView } from '$lib/droppedView';
...
/**
 * A drag whose only flavour is the one a browser fills in for an image source,
 * as recipe 0599's is. It carries no content state and must resolve to nothing.
 */
...
describe('resolveDroppedView', () => {
    it('resolves a bare manifest URL', () => {
        expect(resolveDroppedView(plain(MANIFEST))).toEqual({
            manifestId: MANIFEST,
        });
    });
    ...
    it('resolves the region a content state names', () => {
        expect(
            resolveDroppedView(
                plain(contentStateDocument('#xywh=10,20,30,40')),
            ),
        ).toEqual({
            manifestId: MANIFEST,
            canvasId: CANVAS,
