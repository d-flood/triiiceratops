 ...
 * 3. The palette module and `app.css` say the same thing. The stylesheet cannot
...
function block(needle: string): string {
    const start = APP_CSS.indexOf(needle);
    expect(start, `app.css declares a block for ${needle}`).toBeGreaterThan(-1);
...
describe('the two values that invert between the schemes', () => {
    it('steps the orange lighter in dark, not darker', () => {
        ...
        expect(
            contrast(colour('--link', 'dark'), '#ffffff'),
        ).toBeLessThan(contrast(colour('--link', 'light'), '#ffffff'));
    });

    it('keeps the brand amber off the light ground and admits it on the dark one', () => {
        expect(contrast(colour('--amber', 'light'), colour('--bone', 'light')))
    ...
    it('emphasises with the orange on light and the amber on dark', () => {
        ...
        expect(colour('--mark-emphasis', 'dark')).toBe(colour('--amber', 'dark'));
    });
});
...
describe('app.css and the palette module', () => {
    ...
    it('name no colour token the appendix does not document', () => {
        ...
        for (const name of rootLight.keys()) {
            expect(
                documented.has(name),
                `${name} is declared in app.css but missing from palette.ts`,
            ).toBe(true);
        }
    });
