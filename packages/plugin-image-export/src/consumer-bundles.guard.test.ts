...
async function bundleConsumer(): Promise<string> {
    ...
    writeFileSync(
        entry,
        `export { ImageDownloadPlugin } from ${JSON.stringify(ENTRY)};\n`,
    );
...
describe('the built ESM entry survives a consumer bundle', () => {
    it('has a build to inspect', () => {
        expect(
            existsSync(ENTRY),
            `${ENTRY} is missing — run \`pnpm --filter @triiiceratops/plugin-image-export build\``,
        ).toBe(true);
    });
    ...
    describe('bundled into a host application', () => {
        ...
        it('re-exports ImageDownloadPlugin', () => {
            expect(consumer).toMatch(/\bImageDownloadPlugin\b/);
        });
