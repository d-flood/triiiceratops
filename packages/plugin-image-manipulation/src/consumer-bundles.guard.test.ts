 ...
 *
 * Requires `pnpm --filter @triiiceratops/plugin-image-manipulation build` to have run. It runs in the `node`
...
async function bundleConsumer(): Promise<string> {
    ...
    writeFileSync(
        entry,
        `export { ImageManipulationPlugin } from ${JSON.stringify(ENTRY)};\n`,
    );
...
describe('the built ESM entry survives a consumer bundle', () => {
    it('has a build to inspect', () => {
        expect(
            existsSync(ENTRY),
            `${ENTRY} is missing — run \`pnpm --filter @triiiceratops/plugin-image-manipulation build\``,
        ).toBe(true);
    });
    ...
    describe('bundled into a host application', () => {
        ...
        it('re-exports ImageManipulationPlugin', () => {
            expect(consumer).toMatch(/\bImageManipulationPlugin\b/);
        });
