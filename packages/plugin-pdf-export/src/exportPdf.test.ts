/**
 * The moved export-logic unit tests run under happy-dom (as they did in core),
 * whose `Blob` implements `arrayBuffer()` — the pdf-lib embed path this suite
 * exercises depends on it. The conformance suite keeps the file-default jsdom
...
// Mock the shared public seam `exportPdf` imports from. `composeImages` is
// stubbed (jsdom/happy-dom have no real 2D canvas encoder); everything else is
// the real compiled core util, so the IIIF resolution paths are exercised for
// real. Requires the workspace to be built (`pnpm build:all`) so
// `triiiceratops/image-export` resolves from core's dist.
...
import { composeImages } from 'triiiceratops/image-export';
