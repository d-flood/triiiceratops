...
describe('Annotorious CSS single-source (F23)', () => {
    ...
    it('carries the plugin layer/point-marker fixes in the installed stylesheet', () => {
        // `STYLES` is the combined sheet the style service installs; the fixes
        // (e.g. the point-marker override) ride along with the imported CSS.
        expect(STYLES).toContain('point-selected');
        expect(STYLES).toContain('a9s-');
    });
});
