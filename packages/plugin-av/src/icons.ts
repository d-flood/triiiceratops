/**
 * The pictures this plugin's chrome wears, as {@link IconDescriptor}s core
 * renders.
 *
 ...
 *
 * They are written as finished descriptors rather than authored through the
 * SDK's `svgIcon`, which is ~1.4 KB of validating regex in a bundle measured
 * against a competitive budget (`scripts/size-check.mjs`). That helper earns its
 * bytes over markup a third-party author hands the SDK at runtime; these are seven
 * constants in this repository, and re-scanning them on every page load buys
 * nothing a reviewer has not already done.
 *
 * What the helper guaranteed must therefore hold by hand. Core injects `inner`
 * into an `<svg>` wrapper it owns, so a glyph added here must be the CHILDREN of
 * an `<svg>` root — no wrapper element — and must carry no `<script>`, no
 * `<foreignObject>`, no `on*` handler attribute, and no `href`/`xlink:href` that
 * is not an internal `#id` fragment.
 *
...
/**
 * One Phosphor "regular" glyph. Naming the shared viewBox once is what stops a
 * later addition being scaled against the wrong coordinate system — core sizes
 * the glyph from this string, and Phosphor's is not the `0 0 24 24` most icon
 * sets use.
 */
const phosphor = (inner: string): IconDescriptor => ({
    kind: 'svg',
    viewBox: '0 0 256 256',
    inner,
});

/** The toolbar glyph. Phosphor `FilmStrip`. */
export const FILM_STRIP_ICON: IconDescriptor = phosphor(
...
/** Phosphor `Play`. */
const PLAY_ICON: IconDescriptor = phosphor(
...
/** Phosphor `Pause`. */
const PAUSE_ICON: IconDescriptor = phosphor(
 ...
 */
const SPEAKER_HIGH_ICON: IconDescriptor = phosphor(
...
const SPEAKER_SLASH_ICON: IconDescriptor = phosphor(
...
const CLOSED_CAPTIONING_ICON: IconDescriptor = phosphor(
...
/**
 * Phosphor `Article` — the readable-text control. A page of text, deliberately
 * not a speech bubble or a `CC` badge: what it opens is the transcript panel,
 * and the caption control sits beside it wearing the badge that means captions.
 */
const ARTICLE_ICON: IconDescriptor = phosphor(
    '<path d="M216,40H40A16,16,0,0,0,24,56V200a16,16,0,0,0,16,16H216a16,16,0,0,0,16-16V56A16,16,0,0,0,216,40Zm0,160H40V56H216V200ZM184,96a8,8,0,0,1-8,8H80a8,8,0,0,1,0-16h96A8,8,0,0,1,184,96Zm0,32a8,8,0,0,1-8,8H80a8,8,0,0,1,0-16h96A8,8,0,0,1,184,128Zm-40,32a8,8,0,0,1-8,8H80a8,8,0,0,1,0-16h56A8,8,0,0,1,144,160Z"/>',
);
