...
import {
    ...
    type ViewerHandleClaim,
    type ViewerHandleSlot,
} from '../framework/index.js';

/**
 * One normalized template-ref atom, structurally. Vue's own
 * `VNodeNormalizedRefAtom` is not exported, and only these three members are
 * read, so it is restated here rather than reached for through an internal
 * import.
 */
interface TemplateRefAtom {
    /** The owning component instance — where a STRING ref's box lives. */
    readonly i?: unknown;
    /** The ref itself: a ref object, a name, or a callback. */
    readonly r?: unknown;
    /** Vue's `v-for` marker: refs deliberately collect into an array. */
    readonly f?: boolean;
}

/** Ownership for `:ref="someRef"`, keyed by the ref object itself. */
const slotByRefObject = new WeakMap<object, ViewerHandleSlot>();
/** Ownership for `ref="name"`, keyed by the owning instance, then the name. */
const slotsByOwner = new WeakMap<object, Map<string, ViewerHandleSlot>>();

function atomsOf(vnodeRef: unknown): readonly TemplateRefAtom[] {
    if (!vnodeRef) return [];
    const list = Array.isArray(vnodeRef) ? vnodeRef : [vnodeRef];
    return list.filter(
        (atom): atom is TemplateRefAtom =>
            typeof atom === 'object' && atom !== null,
    );
}

/** The slot standing for one atom's box, or `null` when it has no box. */
function slotForAtom(atom: TemplateRefAtom): ViewerHandleSlot | null {
    // Inside `v-for`, sharing one ref across every match is Vue's documented
    // behaviour, not a wiring mistake.
    if (atom.f) return null;

    const target: unknown = atom.r;
    ...
    if (typeof target === 'string') {
        ...
        if (!byName) {
            byName = new Map<string, ViewerHandleSlot>();
 ...
 * rebind cleanly.
 */
export function claimTemplateRefOwnership(
    instance: ComponentInternalInstance | null,
    element: TriiiceratopsViewerElement,
): () => void {
    const slots = atomsOf(instance?.vnode.ref)
        .map(slotForAtom)
        .filter((slot): slot is ViewerHandleSlot => slot !== null);

    const claims: ViewerHandleClaim[] = [];
    try {
        for (const slot of slots) claims.push(slot.claim(element));
    } catch (error) {
