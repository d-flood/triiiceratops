...
export class TriiiceratopsElementRegistrationError extends Error {
    readonly code = 'ELEMENT_REGISTRATION_UNAVAILABLE' as const;
...
export class TriiiceratopsElementVersionError extends Error {
    readonly code = 'ELEMENT_VERSION_CONFLICT' as const;
    readonly tag: string;

    constructor(tag: string, detail: string) {
        super(
            `<${tag}> is already defined on this page by a constructor with no ` +
...
export class TriiiceratopsHandleConflictError extends Error {
    readonly code = 'VIEWER_HANDLE_CONFLICT' as const;

    constructor(held: string, claiming: string) {
        super(
            `This viewer handle is already bound to ${held} and cannot also be ` +
...
export function describeViewerElement(element: Element | null): string {
