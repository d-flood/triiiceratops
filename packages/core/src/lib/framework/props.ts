...
export interface ViewerElementProps
    extends ViewerAttributeProps, ViewerPropertyProps {}
...
export const VIEWER_PROPERTY_PROPS = [
    ...
    'plugins',
    'searchProvider',
] as const satisfies readonly ViewerPropertyPropName[];

/** The tier of a viewer input, or `undefined` if it is not a viewer input. */
export function viewerPropTier(name: string): ViewerPropTier | undefined {
    if (name in VIEWER_ATTRIBUTE_PROPS) return 'attribute';
    if ((VIEWER_PROPERTY_PROPS as readonly string[]).includes(name)) {
        return 'property';
    }
    return undefined;
}
