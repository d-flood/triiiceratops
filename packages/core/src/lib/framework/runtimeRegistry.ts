...
const runtimeByState = new WeakMap<object, SelectorRuntime>();

/** Publish the wrapper-owned runtime for a viewer state. */
export function attachSelectorRuntime(
    state: ViewerState,
 ...
 */
export function detachSelectorRuntime(
    state: ViewerState,
    runtime: SelectorRuntime,
): void {
 ...
 */
export function getSelectorRuntime(
    state: ReadonlyViewerState | null | undefined,
): SelectorRuntime | undefined {
    if (!state) return undefined;
    return runtimeByState.get(state as object);
}
