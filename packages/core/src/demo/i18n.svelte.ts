/*
 * The demo page chrome's own strings.
 *
 ...
 * element artifact. Locale follows the page-global locale the viewer chrome uses,
 * so the header's language picker moves the demo and the viewer together.
 */
...
const de: Record<DemoMessageKey, string> = {
    ...
    try_your_own: 'Eigenes ausprobieren...',
...
/**
 * A drop-in stand-in for Paraglide's `m` namespace, over the tables above:
 * `m.docs()` reads the current page locale, so the strings re-render when it
 * changes. Unknown locales fall back to English, as Paraglide does.
 */
