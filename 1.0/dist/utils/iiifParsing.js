 ...
 * manifest loads.
 */
const warnedCanvases = new WeakSet();
/**
 ...
 */
function warnUnreadableCanvas(canvas) {
    if (!canvas || typeof canvas !== 'object')
        return;
    if (warnedCanvases.has(canvas))
        return;
    warnedCanvases.add(canvas);
    const id = canvas.id ?? canvas['@id'] ?? '(no id)';
    const spellings = ['images', 'items', 'content'].filter((key) => canvas[key] !== undefined);
    logger.warn(`[triiiceratops] Canvas ${id} yielded no painting annotations, so it will render blank. ` +
        (spellings.length
 ...
 */
function rawSequences(manifest) {
    if (!manifest || typeof manifest !== 'object')
        return [];
    if (isCollectionResource(manifest))
        return [];
    // IIIF v2 (and IxIF).
 ...
 */
function canvasesOfRawSequence(sequence) {
    if (!sequence)
        return [];
    const canvases = sequence.canvases ?? sequence.elements;
    if (canvases)
        return asArray(canvases).filter((canvas) => !!canvas);
 ...
 */
export function getSequenceCount(manifest) {
 ...
 * sibling in this module is public — importing it from `triiiceratops` fails.
 */
export function getCanvasesForSequence(manifest, index) {
 ...
 */
export function getPaintingAnnotations(canvas) {
    if (!canvas)
        return [];
    ...
    const annotations = pages.flatMap((page) => asArray(page?.items).filter((annotation) => !!annotation));
    if (annotations.length === 0)
        warnUnreadableCanvas(canvas);
    return annotations;
}
/**
 ...
 * sibling in this module is public — importing it from `triiiceratops` fails.
 */
export function getPaintingBody(annotation) {
 ...
 * sibling in this module is public — importing it from `triiiceratops` fails.
 */
export function isChoiceBody(body) {
 ...
 * sibling in this module is public — importing it from `triiiceratops` fails.
 */
export function getChoiceAlternatives(body) {
    if (!body)
        return [];
    return [
 ...
 * sibling in this module is public — importing it from `triiiceratops` fails.
 */
export function getCanvasChoices(canvas) {
    if (!canvas)
        return [];
 ...
 * sibling in this module is public — importing it from `triiiceratops` fails.
 */
export function getCanvasBehaviors(canvas) {
