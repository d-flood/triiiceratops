import { buildIiifImageRequestUrl, getRegionString, } from './resolveCanvasImage';
import { createIiifTileSource, isIiifLevel0Profile, getFullImageUrlForLevel, } from '../components/osdTileSources';
...
export function getCompositeImagePlacement(image, canvasWidth, canvasHeight, scale) {
    const width = Math.max(1, Math.round(image.width * canvasWidth * scale));
    const aspect = image.resourceWidth && image.resourceHeight
...
export async function fetchImageBlob(url, requestInit) {
...
async function loadImageElement(blob) {
    ...
    try {
        return await new Promise((resolve, reject) => {
            ...
            element.onerror = () => reject(new Error('Unable to decode image for export.'));
 ...
 */
export async function composeImages(entries, canvasWidth, canvasHeight, format = 'image/png') {
 ...
 */
export function clampCompositeSize(width, height) {
    let scale = 1;
 ...
 */
export function getResolvedImageExportUrl(resolved, options = {}) {
...
async function resolveLevel0SizeOptions(resolved) {
    if (!resolved.serviceId)
        return [];
    ...
    try {
        ...
        const seen = new Set();
        const options = [];
        for (let level = tileSource.minLevel; level <= tileSource.maxLevel; level += 1) {
 ...
 */
export function buildRelativeSizeOptions(nativeWidth, nativeHeight, getUrl) {
...
function resolvePresetSizeOptions(resolved) {
    ...
    return buildRelativeSizeOptions(nativeWidth, nativeHeight, ({ width, height, isOriginal }) => getResolvedImageExportUrl(resolved, isOriginal ? {} : { width, height }));
}
/**
 ...
 */
export async function resolveExportSizeOptions(resolved) {
    if (isIiifLevel0Profile(resolved.serviceProfile)) {
