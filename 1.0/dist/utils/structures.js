...
function getBehaviors(resource) {
 ...
 */
function parseV3Range(range, depth) {
 ...
 */
function parseV2Range(range, depth, allRangesById) {
    const id = range['@id'] || range.id || '';
    const label = resolveLabel(range.label);
    const behaviors = getBehaviors(range);
    const canvasIds = [];
    ...
    if (Array.isArray(range.canvases)) {
        for (const c of range.canvases) {
            const cid = (typeof c === 'string' ? c : c['@id'] || c.id || '').split('#')[0];
            if (cid)
                canvasIds.push(cid);
        }
    }
    ...
    if (Array.isArray(range.members)) {
        for (const member of range.members) {
            ...
            if (memberType === 'sc:Canvas' || memberType === 'Canvas') {
                ...
                children.push(parseV2Range(childRange, depth + 1, allRangesById));
            }
        }
    }
    // Sub-ranges
 ...
 */
export function parseStructures(manifest) {
    if (!manifest)
        return [];
 ...
 */
export function isStructureNodeActive(node, canvasId) {
    if (!canvasId)
        return false;
 ...
 */
export function getSequenceNodeIndexById(nodes, nodeId) {
    const sequenceNodes = nodes.filter((node) => node.behaviors.includes('sequence'));
