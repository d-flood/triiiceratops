...
function appendInlineStyles(element, styles) {
    ...
    element.setAttribute('style', normalizedExisting
...
function normalizeRichTextHtml(html) {
    ...
    for (const anchor of container.querySelectorAll('a')) {
        appendInlineStyles(anchor, 'color: var(--tri-color-primary-text); text-decoration: underline; text-underline-offset: 0.2em;');
    }
    for (const paragraph of container.querySelectorAll('p')) {
        appendInlineStyles(paragraph, 'margin: 0 0 0.75rem;');
    }
    for (const image of container.querySelectorAll('img')) {
        appendInlineStyles(image, 'display: inline-block; max-width: 100%; height: auto; vertical-align: middle; border-radius: 0.25rem;');
    }
    ...
    for (const sub of container.querySelectorAll('sub')) {
        appendInlineStyles(sub, 'font-size: 0.75em; line-height: 0; position: relative; vertical-align: baseline; bottom: -0.2em;');
    }
    for (const sup of container.querySelectorAll('sup')) {
        appendInlineStyles(sup, 'font-size: 0.75em; line-height: 0; position: relative; vertical-align: baseline; top: -0.45em;');
    }
...
export function hasNativeHtmlSanitizer() {
    if (typeof window === 'undefined')
        return false;
    const browserWindow = window;
    return (typeof browserWindow.Sanitizer !== 'undefined' &&
        typeof Element !== 'undefined' &&
        typeof Element.prototype.setHTML === 'function');
}
function sanitizeWithNative(html) {
    const container = document.createElement('div');
...
async function loadDOMPurify() {
    ...
    domPurifyPromise ??= import('dompurify').then(({ default: imported }) => imported(window));
    return domPurifyPromise;
}
export function sanitizeHtmlSync(html) {
    if (!html)
        return '';
    if (!hasNativeHtmlSanitizer())
        return null;
