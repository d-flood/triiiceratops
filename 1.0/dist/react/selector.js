 ...
 */
function useViewerHandleValue(slot) {
...
export function useViewerSelector(handleOrProjection, projectionOrOptions, maybeOptions) {
    ...
    const projection = (contextForm ? handleOrProjection : projectionOrOptions);
    ...
    const bound = useMemo(() => runtime?.createProjection(projection, { equals, cadence }) ?? null, [runtime, projection, equals, cadence]);
    const subscribe = useCallback((onStoreChange) => bound ? bound.subscribe(onStoreChange) : noop, [bound]);
    const getSnapshot = useCallback(() => (bound ? bound.read() : undefined), [bound]);
