...
const ViewerHandleContext = createContext(null);
/**
...
export function ViewerProvider(props) {
    return createElement(ViewerHandleContext.Provider, { value: props.value }, props.children);
}
/**
 ...
 */
export function useResolvedViewerHandle(explicit, hookName) {
    const provided = useContext(ViewerHandleContext);
    const slot = explicit ?? provided;
    if (!slot) {
        throw new Error(`${hookName}() was called with no viewer handle and no ` +
