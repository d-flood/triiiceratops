...
import { TriiiceratopsElementRegistrationError, TriiiceratopsElementVersionError, } from './errors.js';
 ...
 */
export function createViewerElementRegistrar(options) {
    ...
    // already failed.
    let pending;
    return function ensureRegistered() {
        return (pending ??= register(tag, getRegistry, options.load));
    };
}
async function register(tag, getRegistry, load) {
    const registry = getRegistry();
    if (!registry) {
        throw new TriiiceratopsElementRegistrationError(`Cannot register <${tag}>: this environment has no ` +
 ...
 * policy.
 */
export function assertViewerElementCompatible(registry, tag = VIEWER_ELEMENT_TAG) {
    const ctor = registry.get(tag);
    if (!ctor) {
        throw new TriiiceratopsElementVersionError(tag, `Loading the element bundle left <${tag}> undefined.`);
    }
    const prototype = ctor.prototype;
    const bridged = typeof prototype === 'object' &&
        prototype !== null &&
        VIEWER_STATE_BRIDGE_PROPERTY in prototype;
    if (!bridged) {
        throw new TriiiceratopsElementVersionError(tag, `Its constructor prototype has no \`${VIEWER_STATE_BRIDGE_PROPERTY}\` getter.`);
    }
}
/**
 ...
 */
export function ensureViewerElementRegistered() {
