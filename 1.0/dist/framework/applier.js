...
import { shallowEqual, VIEWER_PROPERTY_PROPS, } from './props.js';
/**
 ...
 */
export function createViewerPropApplier(element, options = {}) {
    ...
    function write(prop, value) {
        ...
        if (count > threshold && !warned.has(prop)) {
            warned.add(prop);
            logger.warn(`The \`${prop}\` viewer prop has been re-assigned ${count} times ` +
        ...
        apply(props) {
            for (const prop of VIEWER_PROPERTY_PROPS) {
                ...
                if (applied.has(prop)) {
                    ...
                    if (shallowEqual(applied.get(prop), next))
                        continue;
                }
