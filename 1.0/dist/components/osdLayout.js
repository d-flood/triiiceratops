...
function isPositionedSource(source) {
    return !!source && typeof source === 'object' && 'tileSource' in source;
}
function getDimension(tileSource, key) {
    if (!tileSource || typeof tileSource !== 'object')
        return null;
    const value = tileSource[key];
...
function groupSources(sources) {
    ...
    sources.forEach((source, index) => {
        ...
        const localHeight = imageWidth && imageHeight
...
function useOriginalPositions(groups) {
    return {
        ...
        sources: groups.flatMap((group) => group.sources.map(({ tileSource, localX, localY, localWidth }) => ({
            tileSource,
            x: localX,
            y: localY,
            width: localWidth,
...
export function getCanvasDisplayLayouts(sources, options) {
    ...
    if (options.mode === 'continuous') {
        let offset = 0;
        const isVertical = options.direction === 'top-to-bottom' ||
            options.direction === 'bottom-to-top';
        const isReverse = options.direction === 'right-to-left' ||
            options.direction === 'bottom-to-top';
        ...
        scaled.forEach((layout, index) => {
            ...
            layout.x = previous.reduce((offset, item) => offset + (canNormalize ? item.width : 1) + options.gap, 0);
    ...
    return {
        ...
        sources: scaled.flatMap((layout) => layout.group.sources.map(({ tileSource, localX, localY, localWidth }) => ({
            tileSource,
...
export function getContinuousTargetPosition(indexOrCanvasId, layouts, direction) {
    const layout = typeof indexOrCanvasId === 'number'
