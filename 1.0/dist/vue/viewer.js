 ...
 */
import { defineComponent, getCurrentInstance, h, onActivated, onBeforeUnmount, onDeactivated, onMounted, shallowRef, watch, watchEffect, } from 'vue';
import { createViewerBinding, createViewerPropApplier, VIEWER_ELEMENT_TAG, VIEWER_EVENT_CHANNELS, viewerElementAttributes, } from '../framework/index.js';
...
const viewerEmits = {
    ...
    canvasChange: (snapshot) => {
        void snapshot;
        return true;
    },
    manifestChange: (snapshot) => {
        void snapshot;
        return true;
    },
    choiceChange: (snapshot) => {
        void snapshot;
        return true;
    },
    pluginError: (error) => {
 ...
 */
function attributeTierProps(props) {
    const forced = {};
    for (const [attribute, value] of Object.entries(viewerElementAttributes(props))) {
...
export const TriiiceratopsViewer = defineComponent({
    ...
    setup(props, { attrs, emit, expose }) {
        ...
        const handleRef = shallowRef(null);
        const registrationError = shallowRef(null);
        let controller = null;
        let applier = null;
        let applierElement = null;
        let removeListeners = [];
        let releaseTemplateRef = null;
        /**
        ...
        onMounted(() => {
            ...
            removeListeners = VIEWER_EVENT_CHANNELS.map((channel) => {
                const listener = (event) => {
                    emit(EMIT_BY_CHANNEL[channel], event.detail);
                };
